...
void main() {
  ...
  // Apply a radial fade to the water plane for the Tsunami effect
  float dist = length(modelPosition.xz);
  float maxRadius = 1.0; // This will be passed as a uniform
  float radialFade = 1.0 - smoothstep(maxRadius * 0.9, maxRadius, dist);

  float elevation = getElevation(modelPosition.x, modelPosition.z);
  modelPosition.y += elevation * radialFade;
